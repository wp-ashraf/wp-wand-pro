				...
				async function c() {
						...
						s =
							...
							( Math.random().toString( 36 ).substring( 2, 15 ) +
								Math.random()
									.toString( 36 )
									.substring( 2, 15 ) ) +
							'" class="wpwand-editor-loading">Loading...</span>';
					...
					if ( e.length > 1 || 'core/paragraph' !== n.name ) {
						...
						if (
							! wp.data
								.select( 'core/block-editor' )
								.canInsertBlockType( 'core/paragraph', e )
						) {
							for (
								;
								e &&
					...
					if (
						! wp.data
							.select( 'core/block-editor' )
							.canInsertBlockType( 'core/paragraph', d )
					) {
						for (
							;
							d &&
					...
					const b = wp.richText.slice( u, 0, g ),
					...
					const x = m;
					x[ v ] = s;
					const _ = wp.blocks.createBlock( 'core/paragraph', x ),
						T = m;
					T[ v ] = f;
					let y = [ I, _, wp.blocks.createBlock( n.name, T ) ];
					return (
						0 === k.text.trim().length && ( y = [ I, _ ] ),
						await wp.data
							.dispatch( 'core/block-editor' )
							.replaceBlock( c, y ),
						_
					...
					try {
						r = await ( async function ( t ) {
							...
							e.append( 'prompt', t ),
								e.append(
									'action',
									'wpwand_editor_request'
								) /* , e.append("nonce", wpwand_editor_wp_nonce) */;
							const o = await fetch(
								wpwand_gutenberg_editor.editor_ajax_url,
								{
									method: 'POST',
									body: e,
								}
						...
						return (
							await wp.data
								.dispatch( 'core/block-editor' )
								.removeBlocks( t.clientId ),
							void alert(
								'An API error occurred with the following response body: \n\n' +
									e.message
							)
						);
					}
					...
					return (
						t.forEach( ( t ) => {
							...
							e.clientId === t &&
								...
								( n = n.substring( l, p ) ),
								( r += '\n' + n ),
								c.innerBlocks.length > 0 &&
									( r += s(
										c.innerBlocks.map( ( t ) => t.clientId )
									) );
						} ),
						r
					);
				...
				( 0, e.registerFormatType )( 'wpwand/custom-buttons', {
					...
					edit: ( e ) => {
						let { isActive: n, onChange: l, value: i } = e,
							s = [];
						if (
							'object' === typeof wpwand_gutenberg_editor &&
							'object' ===
								typeof wpwand_gutenberg_editor.editor_menus
						) {
							const t = wpwand_gutenberg_editor.change_action;
							for (
								let e = 0;
								e < wpwand_gutenberg_editor.editor_menus.length;
								e++
							) {
								const o =
									wpwand_gutenberg_editor.editor_menus[ e ];
								void 0 !== o.name &&
									'' !== o.name &&
									void 0 !== o.prompt &&
									'' !== o.prompt &&
						...
						return ( 0, t.createElement )(
							...
							( 0, t.createElement )(
								r.ToolbarGroup,
								null,
								( 0, t.createElement )( r.ToolbarDropdownMenu, {
	...
	( r.m = e ),
		...
		( r.O = ( e, o, c, a ) => {
			if ( ! o ) {
				...
				for ( p = 0; p < t.length; p++ ) {
					for (
						var [ o, c, a ] = t[ p ], l = ! 0, i = 0;
						i < o.length;
						i++
					) {
						( ! 1 & a || n >= a ) &&
						Object.keys( r.O ).every( ( t ) => r.O[ t ]( o[ i ] ) )
							? o.splice( i--, 1 )
							: ( ( l = ! 1 ), a < n && ( n = a ) );
					}
					if ( l ) {
			...
			a = a || 0;
			for ( var p = t.length; p > 0 && t[ p - 1 ][ 2 ] > a; p-- ) {
				t[ p ] = t[ p - 1 ];
			}
			t[ p ] = [ o, c, a ];
		} ),
		...
		( () => {
			...
			const e = ( e, o ) => {
				...
				o = ( globalThis.webpackChunkgutenpride =
					globalThis.webpackChunkgutenpride || [] );
			o.forEach( e.bind( null, 0 ) ),
				( o.push = e.bind( null, o.push.bind( o ) ) );
		} )();
	let c = r.O( void 0, [ 431 ], () => r( 401 ) );
