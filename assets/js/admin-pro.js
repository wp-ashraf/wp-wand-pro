	...
	$( document ).ready( function ( $ ) {
		...
		$( 'a.wpwand-btn.wpwand-prompt-add' ).on( 'click', function ( e ) {
			...
			Swal.fire( {
				...
				html:
					`
                <label class="swal2-input-label" for="wpwand--addprmt-title">` +
					wpwand_pro_glb.addprmt_title_label +
					`</label>
                <input name="wpwand--addprmt-title" id="wpwand--addprmt-title" class="swal2-input" placeholder="` +
					wpwand_pro_glb.addprmt_title_placeholder +
					`" required>
                <label class="swal2-input-label" for="wpwand--addprmt-prompt">` +
					wpwand_pro_glb.addprmt_prompt_label +
					`</label>
                <textarea name="wpwand--addprmt-prompt" id="wpwand--addprmt-prompt" class="swal2-input" placeholder="` +
					wpwand_pro_glb.addprmt_prompt_placeholder +
					`" required></textarea>
                <p>` +
					wpwand_pro_glb.addprmt_prompt_info +
					` </p>
                `,
				...
				preConfirm: ( value ) => {
					...
					const title = document.getElementById(
							'wpwand--addprmt-title'
						).value,
						prompt = document.getElementById(
							'wpwand--addprmt-prompt'
						).value;
				...
				$.post( {
					...
					success( res ) {
						if ( false != res ) {
							$( '#wpwand-ntd-tab-templates table tbody' )
								.append( `
                            <tr data-id="${ res }">
                                    <td>
		...
		$( 'a.wpwand-promtmpt-edit' ).on( 'click', function ( e ) {
			...
			const title = $parent.find( '.wpwand-table-name a' ).text().trim(),
				prompt = $parent
					.find( '.wpwand-table-name .wpwand-data-prompt' )
					.text()
					.trim(),
			...
			Swal.fire( {
				...
				preConfirm: ( value ) => {
					// You can add your custom validation logic here
					const title = document.getElementById(
							`wpwand--updtprmt-title-` + id + ``
						).value,
						prompt = document.getElementById(
							`wpwand--updtprmt-prompt-` + id + ``
						).value;
				...
				$.post( {
					...
					success( res ) {
						...
						if ( false != res ) {
							$parent
								.find( '.wpwand-table-name a' )
								.text( title );
							$parent
								.find(
									'.wpwand-table-name .wpwand-data-prompt'
								)
								.text( prompt );
						}
		...
		// add character
		$( 'a.wpwand-btn.wpwand-prompt-aichar-add' ).on(
			'click',
			function ( e ) {
				e.preventDefault();
				...
				Swal.fire( {
					...
					html:
						`
                <label class="swal2-input-label" for="wpwand--addaichar-title">` +
						wpwand_pro_glb.addaichar_title_label +
						`</label>
                <input name="wpwand--addaichar-title" id="wpwand--addaichar-title" class="swal2-input" placeholder="` +
						wpwand_pro_glb.addaichar_title_placeholder +
						`" required>
                <label class="swal2-input-label" for="wpwand--addaichar-prompt">` +
						wpwand_pro_glb.addaichar_prompt_label +
						`</label>
                <textarea name="wpwand--addaichar-prompt" id="wpwand--addaichar-prompt" class="swal2-input" placeholder="` +
						wpwand_pro_glb.addaichar_prompt_placeholder +
						`" required></textarea>
                `,
					showCloseButton: true,
					closeButtonHtml: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M6 18L18 6M6 6L18 18" stroke="#7C838A" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
					...
					preConfirm: ( value ) => {
						// You can add your custom validation logic here
						const title = document.getElementById(
								'wpwand--addaichar-title'
							).value,
							prompt = document.getElementById(
								'wpwand--addaichar-prompt'
							).value;
					...
					$.post( {
						...
						success( res ) {
							console.log( res );
							if ( false != res ) {
								$( '#wpwand-ntd-tab-aichar table tbody' )
									.append( `
                            <tr data-id="${ res }">
                                    <td>
		...
		$( '#wpwand-select-all' ).on( 'change', function () {
			...
			$( 'input[name="wpwand-select-post[]"]' ).prop(
				'checked',
				isChecked
			);
		} );
		...
		$( 'input[name="wpwand-select-post[]"]' ).on( 'click', function ( e ) {
			...
			if ( e.shiftKey ) {
				const start = $( 'input[name="wpwand-select-post[]"]' ).index(
					this
				);
				const end = $( 'input[name="wpwand-select-post[]"]' ).index(
					lastChecked
				);
		...
		$( 'input[name="wpwand-select-post[]"]' ).on( 'change', function () {
			...
			const checkedCheckboxes = $(
				'input[name="wpwand-select-post[]"]:checked'
			).length;
			$( '#wpwand-select-all' ).prop(
				'checked',
		...
		$( '.wpwand-history-bulk-action input[type="submit"]' ).on(
			'click',
			function ( e ) {
				const $form = $( this ).closest( 'form' );
				const $selectedAction = $( this ).prev( 'select' ).val();
				const $checkedItems = $form.find(
					'input[name="wpwand-select-post[]"]:checked'
				);
